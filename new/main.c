...
void	movie_button(t_doom *den, t_col *vec)
{
	...
	{
		printf("portal \n");
		if (den->find_tmp_one != 0 && den->find_tmp_two != 0 && den->find_tmp->inside == 0)
		{
			portal_sector(den);
			clear_find(den);
			den->change = 0;
...
void	load_map_file(t_wals **tmp, t_doom *den)
{
	int		fd;
	t_wals	*tmp3;
	t_wals	*tmp2;
	...
	den->walls = 0;
	fd = open("sort_map", O_RDONLY);
	tmp3 = 0;
	*tmp = 0;
	tmp2 = (t_wals *)malloc(sizeof(t_wals));
	while (read(fd, tmp2, sizeof(t_wals)))
	{
		if (tmp3 == 0)
		{
			tmp3 = (t_wals *)malloc(sizeof(t_wals));
			*tmp3 = *tmp2;
			*tmp = tmp3;
		}
		else
		{
			tmp3->next = (t_wals *)malloc(sizeof(t_wals));
			*tmp3->next = *tmp2;
			tmp3 = tmp3->next;
			den->walls += 1;
		}
...
void	main3d_edit(t_doom *den)//, t_wals *w)
{
	...
	get_def_new(&den->iw);
	den->iw.win = SDL_CreateWindow("SDL", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
