...
void	draw_between_sectors_top_tex2(t_sdl *iw, t_save_wall *left,
	int *tmp, t_draw_wall_tex *d)
{
	...
	i = iw->d.top[left->x + d->j] - 1;
	while (++i < tmp[d->j] && i < iw->d.bottom[left->x + d->j])
	{
	...
	t_save_wall *right, int *tmp)
{
	...
	while (++d.j < right->x - left->x)
	{
		...
		if (iw->d.wallTop[d.j] < tmp[d.j])
			...
			* (float)(iw->d.top[left->x + d.j] - iw->d.wallTop[d.j])
			/ (float)(iw->d.wallBot[d.j] - iw->d.wallTop[d.j]);
		else
			d.ty = d.zu;
		draw_between_sectors_top_tex2(iw, left, tmp, &d);
	}
}
