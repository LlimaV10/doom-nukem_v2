	...
	t_save_wall *right, t_draw_wall_floor_ceil_tex *d)
{
	...
	d->ang = acosf((d->lv.x * d->rv.x + d->lv.y * d->rv.y) / (sqrtf(d->lv.x * d->
		lv.x + d->lv.y * d->lv.y) * sqrtf(d->rv.x * d->rv.x + d->rv.y * d->rv.y)));
	...
	while (++d->i < iw->d.bottom[left->x + d->j])
	{
		...
		set_pixel2(iw->sur, left->x + d->j, d->i,
			...
			((d->floor.x < 0) ? (((int)(d->floor.x * (float)iw->t[iw->sectors
				...
				+ iw->t[iw->sectors[iw->d.cs].fr.t]->w - 1) : ((int)(d->floor
					.x * (float)iw->t[iw->sectors[iw->d.cs].fr.t]->w) % iw->t[iw->
					sectors[iw->d.cs].fr.t]->w)), ((d->floor.y < 0) ? (((int)(d->floor
						.y * (float)iw->t[iw->sectors[iw->d.cs].fr.t]->h) % iw->t[iw->
